...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def input_output_generation(self, list_path_temp: List[str]): 
        ...
        for path in list_path_temp: 
            npy, _ = self.input_output_loader(path)
            npy = self.preprocessing(npy)
            npy = np.expand_dims(npy,axis=0) # add batch dims
            npy = np.expand_dims(npy,axis=-1) # add channel dims
            
            X_batch.append(npy) 
            y_batch.append(npy)

        return np.concatenate(X_batch, axis=0), np.concatenate(X_batch, axis=0)